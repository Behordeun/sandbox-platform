...
class LogRotationManager:
    ...
    def cleanup_old_archives(self, days_to_keep: int = 365) -> Dict[str, Any]:
        ...
        results = {"deleted_files": [], "errors": [], "space_freed_mb": 0}
        ...
        for log_file in all_files:
            if log_file.is_file():
                try:
                    ...
                    if not stats["oldest_log"] or mod_time < datetime.fromisoformat(
                        stats["oldest_log"]["date"]
                    ):
                        stats["oldest_log"] = {
                            "file": str(log_file),
                    ...
                    if not stats["newest_log"] or mod_time > datetime.fromisoformat(
                        stats["newest_log"]["date"]
                    ):
                        stats["newest_log"] = {
                            "file": str(log_file),
                            "date": mod_time.isoformat(),
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force rotation regardless of size/age"
    )
    parser.add_argument(
        "--cleanup", type=int, metavar="DAYS", help="Clean up archives older than DAYS"
    )
