...
try:
    from config_loader import get_provider_config, get_service_config
except ImportError:
...
class Settings(BaseSettings):
    ...
    def __init__(self, **kwargs):
        ...
        if doja_config:
            self.dojah_api_key = os.getenv("DOJAH_API_KEY") or doja_config.get(
                "api_key", ""
            )
            self.dojah_app_id = os.getenv("DOJAH_APP_ID") or doja_config.get(
                "app_id", ""
            )
