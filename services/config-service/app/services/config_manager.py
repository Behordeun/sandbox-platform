...
from app.models.config import (
    ConfigCreate, ConfigUpdate, ConfigResponse, ConfigVersion, 
    ConfigHistory, ConfigDiff, ConfigStatus, ConfigFormat
)
...
class ConfigManager:
    ...
    async def _store_version(
        ...
        if len(version_record["versions"]) > settings.max_versions:
            version_record["versions"] = version_record["versions"][-settings.max_versions:]
        
        await self.version_storage.set(config_id, version_record)
    
    def _calculate_diff(self, data1: Dict[str, Any], data2: Dict[str, Any]) -> ConfigDiff:
