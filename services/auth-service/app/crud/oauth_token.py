...
class CRUDOAuthToken(CRUDBase[OAuthToken, TokenRequest, TokenResponse]):
    def get_by_access_token(self, db: Session, *, access_token: str) -> Optional[OAuthToken]:
        return db.query(OAuthToken).filter(OAuthToken.access_token == access_token).first()
    
    def get_by_refresh_token(self, db: Session, *, refresh_token: str) -> Optional[OAuthToken]:
        return db.query(OAuthToken).filter(OAuthToken.refresh_token == refresh_token).first()
    
    def get_by_authorization_code(self, db: Session, *, code: str) -> Optional[OAuthToken]:
    ...
    def exchange_code_for_tokens(
        ...
        *, 
        ...
        token_obj.expires_at = datetime.utcnow() + timedelta(seconds=access_token_expires_in)
