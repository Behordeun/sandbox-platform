...
class UserActivityLoggingMiddleware(BaseHTTPMiddleware):
    ...
    def _log_user_activity(
        ...
        if activity_type in ["user_registration", "user_login", "password_reset_request"]:
            log_data["security_event"] = True
            
        if activity_type == "identity_verification":
            log_data["verification_type"] = self._get_verification_type(request.url.path)
    ...
    def _get_activity_type(self, method: str, path: str) -> str:
        ...
        elif path in ["/api/v1/auth/login", "/api/v1/auth/login/json"] and method == "POST":
            return "user_login"
